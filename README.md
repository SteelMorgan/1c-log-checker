...
**Что хранится:**
- Ошибки за последние 48 часов
- Источники: `event_log` (level='Error') и `tech_log` (level='ERROR', name='EXCP')
- Дедупликация по сигнатуре ошибки

**Поля:**
- `cluster_guid`, `infobase_guid` — идентификация базы
- `event_name` — тип события
- `error_text` — текст ошибки
- `error_signature` — хеш-сигнатура для дедупликации
- `occurrences` — количество повторений
- `first_seen`, `last_seen` — первое и последнее появление
- `sources` — источники ошибки (['event_log'], ['tech_log'] или оба)
- `sample_lines` — примеры записей

**Обновление:** автоматически каждые 10 минут (настраивается через `NEW_ERRORS_UPDATE_INTERVAL`)
