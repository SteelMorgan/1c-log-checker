...
- **MAX_GLOBAL_WORKERS**: 0 (0/отрицательное — нет глобального лимита, >0 — общий семафор на все reader/tailer)
...
1. **Количество потоков зависит от**:
   ...
   - Значений `MAX_WORKERS` и `MAX_GLOBAL_WORKERS`
...
3. **Для контроля нагрузки**:
   ...
   - Включить `MAX_GLOBAL_WORKERS`, если нужен общий предел параллелизма
