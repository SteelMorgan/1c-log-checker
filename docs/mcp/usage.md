...
3. `get_actual_log_timestamp` — максимальная отметка времени в логах для базы

### Управление конфигурацией техжурнала:
4. `save_techlog` — сохранение текущей конфигурации как backup (.OLD)
5. `configure_techlog` — создание новой конфигурации logcfg.xml
6. `get_techlog_config` — чтение текущей конфигурации
7. `restore_techlog` — восстановление конфигурации из backup (.OLD)
8. `disable_techlog` — отключение техжурнала
...
| `level` | string | Нет | Фильтр по уровню (Error/Ошибка, Warning/Предупреждение, Information/Информация, Note/Примечание). Поддерживаются оба варианта (русский и английский), поиск выполняется по обоим |
...
1. Используйте `get_event_log` с фильтром — детали по конкретной ошибке
2. Используйте `get_tech_log` — техническая информация (SQL, блокировки)
...
3. **Комбинируйте инструменты** — используйте get_event_log и get_tech_log для детального анализа
